...
function finishAlarm() {
    const banner = document.getElementById('hostAlarmBanner');
    const file = document.getElementById('hostAlarmBannerFile').innerText;

    banner.classList.add('hidden');
    alarmAudio.pause();

    // Auto-Delete TTS Files if they were played via this alarm mechanism
    if (file && file.startsWith('tts/')) {
        // Send delete request
        const fd = new FormData();
        fd.append('action', 'delete');
        fd.append('file', file);
        fetch('api_files.php', { method: 'POST', body: fd }).catch(e => console.error("Auto-delete failed", e));
    }
...
function showTab(tab) {
    ...
    document.getElementById('tab-tts').classList.add('hidden');
    toggleGradient(false);
    document.getElementById('tab-' + tab).classList.remove('hidden');
    if (tab === 'users') loadAuthorizedUsers();
}
