...
function checkAlarms($alarmsFile) {
    if (!file_exists($alarmsFile)) return ['event' => null, 'trigger_pause' => false];
    $fp = fopen($alarmsFile, 'c+');
    if (!$fp) return ['event' => null, 'trigger_pause' => false];
    $eventResult = null; $shouldPause = false;
    if (flock($fp, LOCK_EX)) {
        $fsize = filesize($alarmsFile);
        $json = $fsize > 0 ? fread($fp, $fsize) : '{}';
        $alarms = json_decode($json, true) ?? ['slots'=>[], 'last_event'=>null];
        try { $tz = new DateTimeZone('Asia/Singapore'); } catch(Exception $e) { $tz = new DateTimeZone('UTC'); }
        $nowObj = new DateTime('now', $tz); $today = $nowObj->format('Y-m-d'); $hhmm  = $nowObj->format('H:i');
        $changed = false;
        foreach ($alarms['slots'] as &$s) {
            ...
            if (($s['time'] ?? '') === $hhmm && ($s['last_fired'] ?? null) !== $today) {
                ...
                $s['last_fired'] = $today; $changed = true; $shouldPause = true; 
            }
        }
        if (isset($alarms['last_event']) && (time() - ($alarms['last_event']['fired_at']??0) < 45)) $eventResult = $alarms['last_event'];
        if ($changed) { ftruncate($fp, 0); rewind($fp); fwrite($fp, json_encode($alarms, JSON_PRETTY_PRINT)); fflush($fp); }
        flock($fp, LOCK_UN);
    }
    fclose($fp);
...
function withStateLock($file, callable $callback) {
    ...
    if (flock($fp, LOCK_EX)) {
        ...
        foreach($defaults as $k=>$v) { if(!isset($data['global'][$k])) $data['global'][$k] = $v; }

        $newData = $callback($data);

        ftruncate($fp, 0); rewind($fp); fwrite($fp, json_encode($newData, JSON_PRETTY_PRINT)); fflush($fp); flock($fp, LOCK_UN); 
    }
    fclose($fp);
    return $newData ?? [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $state = withStateLock($stateFile, function($d) use ($clientId, $clientName, $clientDiff) {
        if ($clientId) {
            ...
            if ($clientName) $d['clients'][$clientId]['name'] = $clientName;
            elseif (!isset($d['clients'][$clientId]['name'])) $d['clients'][$clientId]['name'] = "Device " . substr($clientId, 0, 4);
            if ($clientDiff !== null) $d['clients'][$clientId]['diff'] = floatval($clientDiff);

            // Sync new clients immediately
            if ($isNew || empty($d['clients'][$clientId]['track'])) {
                foreach(['track','status','start_ts','pause_seek','volume'] as $k) 
                    $d['clients'][$clientId][$k] = $d['global'][$k];
            }
        }
        foreach (($d['clients']??[]) as $k => $c) {
            if (time() - ($c['last_seen']??0) > 15) unset($d['clients'][$k]);
        }
        return $d;
    });
    ...
    if (!isset($response['global'])) $response['global'] = $state['global'];
    ...
    withStateLock($stateFile, function($state) use ($action, $reqTrack, $seekVal, $target, $queueJson) {
        ...
        $updateObj = function(&$g) use ($action, $reqTrack, $seekVal, $now, $queueJson) {
            ...
            if ($queueJson) {
                ...
                if (is_array($decoded)) $g['queue'] = $decoded;
            }

            if ($action === 'play_at') {
                if ($reqTrack) $g['track'] = $reqTrack;
                $g['status'] = 'playing';
            ...
            elseif ($action === 'pause') {
                if (($g['status']??'') === 'playing') $g['pause_seek'] = $now - ($g['start_ts']??$now);
                $g['status'] = 'paused';
            }
            elseif ($action === 'stop') {
                $g['status'] = 'stopped'; $g['start_ts'] = 0; $g['pause_seek'] = 0;
            }
            // --- NEW: RESTART LOGIC ---
            ...
            elseif ($action === 'seek') {
                 if ($seekVal !== null) {
                    $g['start_ts'] = $now - (float)$seekVal;
                    if (($g['status']??'') === 'paused') $g['pause_seek'] = (float)$seekVal;
                }
            }
            // --- NEW: THE "DJ" LOGIC ---
